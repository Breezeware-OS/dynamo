...
public class UserManagementService {
    ...
    private Group retrieveGroup(String groupName) {
        ...
        Group group = groupService.retrieveGroup(groupName)
                .orElseThrow(() -> new DynamoException("Group with name '%s' not found".formatted(groupName),
                        HttpStatus.NOT_FOUND));
    ...
    private Role retrieveRole(String roleName) {
        ...
        Role role = roleService.retrieveRole(roleName)
                .orElseThrow(() -> new DynamoException("Role with name '%s' not found".formatted(roleName),
                        HttpStatus.NOT_FOUND));
    ...
    private void validateUpdateUserRequest(UserUpdateRequest userUpdateRequest) {
        ...
        UserStatus userStatus = UserStatus.retrieveUserStatus(userUpdateRequest.getStatus())
                .orElseThrow(() -> new DynamoException(
                ("Invalid user status '%s' for user'. " + "Possible user status are: '%s'").formatted(
                        userUpdateRequest.getUserId(), UserStatus.retrieveAllUserStatus()),
        ...
        if (Objects.nonNull(filterByStatus) && !filterByStatus.isBlank()) {
            ...
            UserStatus userStatus =
                    UserStatus.retrieveUserStatus(filterByStatus)
                            .orElseThrow(() -> new DynamoException(
                            "Invalid status '%s' for user. Possible status values are: '%s'".formatted(
                                    filterByStatus, UserStatus.retrieveAllUserStatus()),
                                    HttpStatus.BAD_REQUEST));
    ...
    public User createUser(SignedUpUserData signedUpUserData) throws DynamoException {
        ...
        Organization organization = organizationService.retrieveOrganization(organizationName)
                .orElseThrow(() -> new DynamoException(
                "No organization with name: %s found".formatted(organizationName),
                        HttpStatus.BAD_REQUEST));
        // add organization for the user
        buildAndPersistUserOrganizationMap(savedUser, organization);
        ...
        groups.forEach(g -> {
            Group group = groupService.retrieveGroup(g)
                    .orElseThrow(() -> new DynamoException("No group with name: %s found".formatted(g),
                            HttpStatus.BAD_REQUEST));
            buildAndPersistUserGroupMap(group, savedUser);
        });
